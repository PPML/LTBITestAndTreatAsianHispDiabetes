...
ltbiPrevSimAsian <- (igra_prev_sims[indexAsian,] - 1 + spec_usb) / (sens_usb + spec_usb - 1)
tempPrevAsian <- apply(X = ltbiPrevSimAsian, MARGIN = 1, FUN = quantile, probs=c(.025, .50, 0.975))
ltbiPrevAsian <- cbind(new_dat[indexAsian,], t(tempPrevAsian))

##### LTBI prevalence Hispanic
indexHispanic <- which(new_dat$`Race-ethnicity`=="Hispanic")
ltbiPrevSimHispanic <- (igra_prev_sims[indexHispanic,] - 1 + spec_usb) / (sens_usb + spec_usb - 1)
tempPrevHispanic <- apply(X = ltbiPrevSimHispanic, MARGIN = 1, FUN = quantile, probs=c(.025, .50, 0.975))
ltbiPrevHispanic <- cbind(new_dat[indexHispanic,], t(tempPrevHispanic))

##### Create a single dataframe 
ltbiPrev <- rbind(ltbiPrevAsian, ltbiPrevHispanic)
...
ltbiPrev$`Population` <- case_when(ltbiPrev$`Diabetes status` == "Diabetes" ~ "Population with diabetes", 
                                   ltbiPrev$`Diabetes status` == "None"   ~  "Population without diabetes", 
                                   ltbiPrev$`Diabetes status` %in% c("Gestational", "Unknown") ~ ltbiPrev$`Diabetes status`)
