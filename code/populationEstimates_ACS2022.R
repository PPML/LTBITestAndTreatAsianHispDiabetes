...
if(is_extract_ready(Asian_ext_submitted)==TRUE){
    ...
    print("Extract is not ready for download. Try again in a few minutes.")
}
...
# print(paste("Total nusbAsian population is", sum(nusbAsianPop$PERWT)/1e6, "million"))
...
Hispanic_ext_def <- define_extract_usa(
    ...
    variables = list(
        "AGE", "CITIZEN",
        var_spec("HISPAN", case_selections = "1"), ### Hispanic
        "REPWT"
    )
)
### Submit the extract request
...
# # print(paste("Total nusbHispanic population is", sum(nusbHispanicPop$PERWT)/1e6, "million"))
# 
# ### Output percentage of population over 65 ###
